...
bool SD_Card_Mounting()
{
    ...
    if (!GSheet.sdBegin(&sdFatSPIConfig, SPI_CS_PIN, SPI_SCK_PIN, SPI_MISO_PIN, SPI_MOSI_PIN)) // pointer to SdSpiConfig, SS, SCK,MISO, MOSI
    {
    ...
    if (!GSheet.sdBegin(SPI_CS_PIN, &spi))                        // SS, pointer to SPIClass <- SPIClass object should defined as static or global
    {
        Serial.println("failed\n");
        return false;
    }
    ...
    if (!GSheet.sdBegin(SPI_CS_PIN)) // or GSheet.sdBegin(&sdFSConfig)
    {
        Serial.println("failed\n");
        return false;
    }
    ...
    if (!GSheet.sdBegin(&sdFSConfig)) // We begin with the SDFSConfig to use SPI 1 port
    {
        Serial.println("failed\n");
        return false;
    }
    ...
    if (!GSheet.sdMMCBegin("/sdcard", false, true))
    {
        Serial.println("failed\n");
        return false;
    }
